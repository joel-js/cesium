...
export var CesiumMeasurer = (function () {
  //constructor
  ...
  _.prototype._addVertexEllipsoidEntity = function(vertexCartesian) {
      ...
      var material = Color.YELLOW.withAlpha(1);

      if(this._vertexEntities.length === 0)
          material = Color.MAROON.withAlpha(1);
  ...
  _.prototype.stopDrawing = function() {
      ...
      var transform = Transforms.eastNorthUpToFixedFrame(normalArrowStartPosition);

      var invTransform = Matrix4.inverseTransformation(transform, new Matrix4());

      var transformedNormalArrowEndPosition = Matrix4.multiplyByPoint(invTransform, normalArrowEndPosition, new Cartesian3());

      var heading = Cartesian3.angleBetween(new Cartesian3(0, 1, 0), new Cartesian3(transformedNormalArrowEndPosition.x, transformedNormalArrowEndPosition.y, 0));

      // calc pitch

      normalArrowStartPosition = Cartesian3.normalize(normalArrowStartPosition, new Cartesian3());

      var pitch = Cartesian3.angleBetween(normalArrowStartPosition, normal);

      pitch = Math.PI_OVER_TWO - pitch;

      heading = Math.toDegrees(heading).toFixed(2);
      pitch = Math.toDegrees(pitch).toFixed(2);
  ...
  _.prototype._collectPositionsInPolygon = function() {
      ...
      for (var i = 0; i < cartesians.length; i++) {
          var carto  = Ellipsoid.WGS84.cartesianToCartographic(cartesians[i]);

          var longitude = Math.toDegrees(carto.longitude);
          var latitude = Math.toDegrees(carto.latitude);
      ...
      if(this._considerInterior) {
          ...
          for (longitude = minLongitude; longitude <= maxLongitude; longitude += gap) {
              for (latitude = minLatitude; latitude <= maxLatitude; latitude += gap) {
                  ...
                  cartographic = Cartographic.fromDegrees(longitude, latitude, height);

                  cartesian = Ellipsoid.WGS84.cartographicToCartesian(cartographic);

                  var rayPosition = cartesian;

                  var rayDirection = Cartesian3.normalize(cartesian, new Cartesian3());

                  rayDirection = Cartesian3.negate(rayDirection, new Cartesian3());
                  ...
                  if(this._debugSamplePositions) {
                      var sphere = this._cesiumViewer.entities.add({
                          ...
                          ellipsoid : {
                              radii : new Cartesian3(debugSphereRadius, debugSphereRadius, debugSphereRadius),
                              material : material
                          }
                      });
      ...
      var cartographic = Cartographic.fromDegrees(minLongitude, minLatitude, 0);

      var cartesianA = Ellipsoid.WGS84.cartographicToCartesian(cartographic);

      cartographic = Cartographic.fromDegrees(maxLongitude, minLatitude, 0);

      var cartesianB = Ellipsoid.WGS84.cartographicToCartesian(cartographic);

      var width = Cartesian3.distance(cartesianA, cartesianB);

      cartographic = Cartographic.fromDegrees(minLongitude, minLatitude, 0);

      cartesianA = Ellipsoid.WGS84.cartographicToCartesian(cartographic);

      cartographic = Cartographic.fromDegrees(minLongitude, maxLatitude, 0);

      cartesianB = Ellipsoid.WGS84.cartographicToCartesian(cartographic);

      height = Cartesian3.distance(cartesianA, cartesianB);
  ...
  _.prototype._calcBestFitPlane = function(cartesians, width, height) {
      ...
      var normal = new Cartesian3(resultArray[0], resultArray[1], 1);

      var distance = resultArray[2] / Cartesian3.magnitude(normal);

      normal = Cartesian3.normalize(normal, new Cartesian3());

      var planePosition = new Cartesian3(centerX, centerY, centerZ);
      ...
      var extendedNormalVector = Cartesian3.multiplyByScalar(normal, normalArrowLength, new Cartesian3());

      var normalPosition = Cartesian3.add(planePosition, extendedNormalVector, new Cartesian3());
      ...
      normalInPlaneFrame = Cartesian3.normalize(normalInPlaneFrame.clone(), new Cartesian3());

      this._plane = this._cesiumViewer.entities.add({
          name : 'best  plane',
          position: planePosition,
          plane : {
              plane : new Plane(normalInPlaneFrame, 0),
              dimensions : new Cartesian2(width, height),
  ...
  _.prototype.transformedPoint = function(center, point) {
      ...
      point = Matrix4.multiplyByPoint(invTransform, point, new Cartesian3());
